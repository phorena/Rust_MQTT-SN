...
type AcceptDoneCh = (
    mpsc::Receiver<(Arc<UdpConn>, Option<State>)>,
    mpsc::Receiver<()>,
);
...
struct ListenerImpl {
    ...
    accept_ch_tx: Arc<Mutex<Option<mpsc::Sender<(Arc<UdpConn>, Option<State>)>>>>,
...
impl Listener for ListenerImpl {
    ...
    //webrtc_util update
    async fn accept(&self) -> Result<(Arc<dyn Conn + Send + Sync>, SocketAddr, Option<State>)> {
        let (accept_ch_rx, done_ch_rx) = &mut *self.ch_rx.lock().await;

        tokio::select! {
            c = accept_ch_rx.recv() =>{
                if let Some((c, initial_state)) = c{
                    //webrtc_util update
                    let raddr = c.raddr;
                    Ok((c, raddr, initial_state))
                }else{
...
pub async fn listen(laddr: &str, db: Option<Arc<ConnectionDb>>) -> Result<impl Listener> {
    ListenConfig::default().listen(laddr, db).await
}
...
impl ListenConfig {
    ...
    pub async fn listen(
        ...
        //Uncomment the first 2 commands below to fix the datatype issue
        //let newladdr: SocketAddr = laddr.parse().unwrap();
        //let pconn = Arc::new(MioUdpSocket::bind(newladdr).await);
        //let pconn = Arc::new(MioUdpSocket::bind(laddr).await?);
        ...
        let pconn_clone = Arc::clone(&pconn);
        let conns_clone = Arc::clone(&conns);
        //let accept_ch_tx_clone = Arc::clone(&accept_ch_tx);
        ...
        let _ = ListenConfig::restore(pconn, &accept_ch_tx, conns, db).await;
    ...
    async fn get_udp_conn(
        ...
        let udp_conn = Arc::new(UdpConn::new(Arc::clone(pconn), Arc::clone(conns), raddr));
        {
            ...
            if let Some(tx) = &*accept_ch {
                if tx.try_send((Arc::clone(&udp_conn), None)).is_err() {
    ...
    // Iterate through the whole database and restore all connections
    pub async fn restore(
        ...
        let mut iter = db.unwrap().to_iter();
        while let Some(Ok((key, value))) = iter.next() {
            //dbg!(key.clone());
            //dbg!(value);
            let key: SocketAddr = match bincode::deserialize(&key) {
                Ok(raddr) => raddr,
                Err(e) => {
            ...
            log::info!("Restoring connection from {}", key);
            // let value: SerializedState = bincode::deserialize(&value).unwrap();
            // let key: Result<SocketAddr, std::boxed::Box<bincode::ErrorKind>> =
            ...
            let _ = initial_state.unmarshal_binary(&value).await; // Restore state from db

            let udp_conn = Arc::new(UdpConn::new(Arc::clone(&pconn), Arc::clone(&conns), key));
            {
                let accept_ch = accept_ch_tx.lock().await;
                if let Some(tx) = &*accept_ch {
                    if tx
                        .try_send((Arc::clone(&udp_conn), Some(initial_state)))
                        .is_err()
                    {
                        return Err(Error::ErrListenQueueExceeded.into());
                    }
            ...
            {
                let mut m = conns.lock().await;
                m.insert(key.to_string(), Arc::clone(&udp_conn));
            }
            //Ok(Some(udp_conn))
        }
        Ok(())
