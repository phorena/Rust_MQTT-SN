...
async fn create_test_server(
    ...
    DTLSConn::new(cb, cfg, false, None, None).await
}
...
async fn test_routine_leak_on_close() -> Result<()> {
    ...
    let n_b = cb.read(&mut buf_b, Some(Duration::from_secs(5))).await?; //Update
    assert_eq!(n_a, 100);
...
async fn test_psk() -> Result<()> {
    ...
    for (name, server_identity) in tests {
        ...
        tokio::spawn(async move {
            let conf = Config {
                psk: Some(Arc::new(psk_callback_client)), //Update
...
async fn test_psk_hint_fail() -> Result<()> {
    ...
    tokio::spawn(async move {
        let conf = Config {
            psk: Some(Arc::new(psk_callback_hint_fail)), //Update
...
async fn test_client_certificate() -> Result<()> {
    ...
    srv_ca_pool
        .add(&srv_cert.certificate[0]) //Update
    ...
    let tests = vec![
        ...
        (
            ...
            Config {
                ...
                client_cas: ca_pool.clone(), //Update
                ..Default::default()
            },
            false,
        ),
    ...
    for (name, client_cfg, server_cfg, want_err) in tests {
        ...
        tokio::spawn(async move {
            let result = DTLSConn::new(Arc::new(ca), client_cfg_clone, true, None, None).await;
            let _ = client_res_tx.send(result).await;
        });
        // let db=Arc::new(ConnectionDb::new("DB".to_string()).unwrap());
        let result = DTLSConn::new(Arc::new(cb), server_cfg.clone(), false, None, None).await;
        let client_result = client_res_rx.recv().await;
            ...
            || server_cfg.client_auth == ClientAuthType::RequireAndVerifyClientCert
        {
            ...
            assert_eq!(
                ...
                client_cfg.certificates[0].certificate[0].as_ref(), //Update
...
async fn test_server_certificate() -> Result<()> {
    ...
    let mut ca_pool = rustls::RootCertStore::empty(); //Update
    ca_pool
        .add(&cert.certificate[0])
        .or_else(|_err| Err(Error::new("add cert error".to_owned())))?;
    ...
    let tests = vec![
        ...
        (
            "good_ca",
            Config {
                roots_cas: ca_pool.clone(), //Update
                server_name: server_name.clone(),
                ..Default::default()
            },
            Config {
        ...
        (
            ...
            Config {
                certificates: vec![cert.clone()],
                client_auth: ClientAuthType::RequireAnyClientCert,
                verify_peer_certificate: Some(Arc::new(fn_not_expected_chain)), //Update
                ..Default::default()
            },
            false,
        ),
        (
            ...
            Config {
                ...
                verify_peer_certificate: Some(Arc::new(fn_expected_chain)), //Update
                ..Default::default()
            },
            false,
        ),
        (
            ...
            Config {
                roots_cas: ca_pool.clone(), //Update
                server_name: server_name.clone(),
                verify_peer_certificate: Some(Arc::new(fn_wrong_cert)), //Update
                ..Default::default()
            },
            Config {
                certificates: vec![cert.clone()],
    ...
    for (name, client_cfg, server_cfg, want_err) in tests {
        ...
        tokio::spawn(async move {
            let result = DTLSConn::new(Arc::new(cb), server_cfg, false, None, None).await;
            let _ = res_tx.send(result).await;
        });

        let cli_result = DTLSConn::new(Arc::new(ca), client_cfg, true, None, None).await;
...
async fn test_psk_configuration() -> Result<()> {
    ...
    ) in tests
    {
        ...
        tokio::spawn(async move {
            let conf = Config {
                psk: if client_psk {
                    Some(Arc::new(psk_callback)) //Update
                } else {
                    None
                },
