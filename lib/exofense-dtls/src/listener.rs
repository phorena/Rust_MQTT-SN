...
use crate::udp_conn::conn_udp_listener::*;
use crate::udp_conn::*;
//use crate::util_replay_detector::*;
...
pub async fn listen(
    ...
    let parent = Arc::new(lc.listen(laddr, db).await?);
    Ok(DTLSListener {
        parent,
        config,
        db: db2,
...
impl DTLSListener {
    ...
    pub fn new(
        ...
        Ok(DTLSListener { parent, config, db })
    }
}
...
impl Listener for DTLSListener {
    ...
    async fn accept(&self) -> Result<(Arc<dyn Conn + Send + Sync>, SocketAddr, Option<State>)> {
        //v0.4.10
        let db2 = match self.db.as_ref() {
            Some(db) => Some(Arc::clone(&db)),
            None => None,
        };
        let (conn, raddr, initial_state) = self.parent.accept().await?; //v0.4.10
        let dtls_conn = DTLSConn::new(conn, self.config.clone(), false, initial_state, db2).await?;
        Ok((Arc::new(dtls_conn), raddr, None)) //v0.4.10
    }
