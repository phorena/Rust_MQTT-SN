...
pub struct State {
    ...
    //pub(crate) db:Arc<Option<ConnectionDb>>,
...
pub struct SerializedState {
    ...
    pub local_random: [u8; HANDSHAKE_RANDOM_LENGTH],
    pub remote_random: [u8; HANDSHAKE_RANDOM_LENGTH],
...
impl State {
    pub(crate) async fn clone(&self) -> Self {
        ...
        //log::info!("WHILE CLONING {:?}", state.serialize().await);

        state
    }

    pub(crate) async fn serialize(&self) -> Result<SerializedState> {
    ...
    pub async fn deserialize(&mut self, serialized: &SerializedState) -> Result<()> {
        // Set epoch values
        self.local_epoch
            .store(serialized.local_epoch, Ordering::SeqCst); // v0.4.10
        self.remote_epoch
            .store(serialized.remote_epoch, Ordering::SeqCst); // v0.4.10
        {
